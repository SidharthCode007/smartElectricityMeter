...
import 'package:smartmeter/screens/userregstration.dart';
import 'package:smartmeter/screens/widgets/logintextfield.dart';
import 'package:smartmeter/services/loginApi.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  loginCheck() async{
   ...
   //
    final res = await userLoginApi(usernameController.text, passwordController.text);
    if (res!=null) {
      if (res.type=="user") {
        loginid = res.lid.toString();
        consumerno = usernameController.text;
        final shared = await SharedPreferences.getInstance();
        shared.setBool("islogedin", true);
        shared.setString("loginId", res.lid.toString());
        shared.setString("consumerId", usernameController.text);
